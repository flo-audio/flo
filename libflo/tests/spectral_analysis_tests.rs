...
fn test_spectral_fingerprint_consistency() {
    ...
    assert_eq!(
        fingerprint1.frequency_resolution,
        fingerprint2.frequency_resolution
    );
    ...
    assert_eq!(
        fingerprint1.spectral_data.len(),
        fingerprint2.spectral_data.len()
    );
    for (frame1, frame2) in fingerprint1
        .spectral_data
        .iter()
        .zip(fingerprint2.spectral_data.iter())
    {
