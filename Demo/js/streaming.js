  ...
  try {
    ...
    if (streamingDemo) {
      ...
      log(
        ...
        } frames in ${elapsed.toFixed(0)}ms`,
        "success",
      );
    }
  } catch (err) {
  ...
  try {
    ...
    if (!streamingDemo.info && decoder.is_ready()) {
      ...
      if (streamingDemo.info) {
        ...
        log(
          `Stream info: ${sample_rate}Hz, ${channels}ch, ${
...
async function decodeAndScheduleFrames() {
  ...
  if (streamingDemo?.active && streamingDemo.sampleBuffer.length > 0) {
    const shouldFlush =
      streamingDemo.bufferedSamples >= MIN_BUFFER_SAMPLES ||
...
async function processRemainingFrames() {
  ...
  if (streamingDemo?.playing) {
    const remaining =
      streamingDemo.nextPlayTime - streamingDemo.audioCtx.currentTime;
    if (remaining > 0) {
...
function updateStreamingProgress() {
  ...
  if (progressEl && streamingDemo) {
    ...
    const percent = streamingDemo.fileSize
      ? Math.round(
          (streamingDemo.totalBytesReceived / streamingDemo.fileSize) * 100,
        )
      : 0;
...
export async function debugCompareDecodeMethods(file) {
  ...
  if (available.length !== streamed.length) {
    console.error(
      `LENGTH MISMATCH: decode_available=${available.length}, next_frame=${streamed.length}`,
    );
  }
  ...
  for (let i = 0; i < minLen; i++) {
    ...
    if (diff > 0.0001) {
      if (diffCount < 10) {
        console.log(
          `Sample ${i}: available=${available[i]}, streamed=${streamed[i]}, diff=${diff}`,
        );
      }
      diffCount++;
    }
  ...
  console.log(
    `NaN/Infinity: available=${availableNaN}, streamed=${streamedNaN}`,
  );
  ...
  console.log(
    `Range: available=[${availableMin.toFixed(4)}, ${availableMax.toFixed(
      4,
    )}], streamed=[${streamedMin.toFixed(4)}, ${streamedMax.toFixed(4)}]`,
  );
