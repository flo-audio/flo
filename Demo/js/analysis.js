...
export async function analyzeAudio(samples, sampleRate, channels) {
  ...
  try {
    ...
    if (extractWaveformPeaks) {
      results.waveform = extractWaveformPeaks(
        samples,
        channels,
        sampleRate,
        100,
  ...
  try {
    ...
    if (extractSpectralFingerprint) {
      results.spectral = extractSpectralFingerprint(
        samples,
        channels,
        sampleRate,
        null,
...
export function drawSpectrumBars(canvas, energyProfile) {
  if (!canvas || !energyProfile) {
    console.log("Spectrum bars: missing canvas or energyProfile", {
      canvas: !!canvas,
  ...
  const profile = Array.isArray(energyProfile)
    ? energyProfile
    : Array.from(energyProfile);

  console.log("Drawing spectrum bars:", {
    length: profile.length,
    values: profile.slice(0, 4),
  });
...
export function updateAnalysisPanel(analysis) {
  ...
  if (analysis?.loudness) {
    ...
    if (integratedEl)
      integratedEl.textContent = formatLUFS(loudness.integrated_lufs);
  ...
  if (analysis?.duration) {
    ...
    if (durationEl) {
      ...
      durationEl.textContent =
        mins > 0 ? `${mins}:${secs.padStart(4, "0")}` : `${secs}s`;
    }
  }
}
