  ...
  try {
    ...
    updateStats({
      ...
      originalSize: Math.round(
        fileInfo.duration_secs * fileInfo.sample_rate * fileInfo.channels * 2,
      ),
...
export async function encodeAndUpdateUI() {
  ...
  try {
    ...
    log(
      `Encoded in ${encodeTime.toFixed(
        1,
      )}ms (${fileInfo.compression_ratio.toFixed(2)}x compression)`,
      "success",
    );
  } catch (err) {
