...
export function populateMetadataEditor(floData) {
  try {
    ...
    if (meta.track_number)
      document.getElementById("metaTrack").value = meta.track_number;
...
export function displayMetadata(floData) {
  ...
  if (!metadataCard || !metadataContent) {
    console.warn(
      "displayMetadata: Missing metadataCard or metadataContent elements",
    );
    return;
  }
...
function displayMetadataObject(meta, floData) {
  ...
  if (floData) {
    try {
      ...
      // get_cover_art returns an object { mime_type, data } or null
      if (coverData && coverData.data && coverData.data.length > 0) {
        const mime = coverData.mime_type || "image/jpeg";
        const blob = new Blob([coverData.data], { type: mime });
    ...
    try {
      ...
      // get_synced_lyrics returns an array of synced-lyrics objects
      if (lyrics && Array.isArray(lyrics) && lyrics.length > 0) {
        // pick first track with lines
        let lines = null;
        for (const track of lyrics) {
          if (track && track.lines && track.lines.length > 0) {
            lines = track.lines;
            break;
          }
        }
        if (lines) {
          const preview = lines
            .slice(0, 5)
            .map((l) => `${formatTimeMs(l.timestamp_ms)}: ${l.text}`)
            .join("\n");
          html += `<div class="meta-row"><span class="meta-label">Synced Lyrics</span><pre class="lyrics-preview">${preview}${
            lines.length > 5 ? "\n..." : ""
          }</pre></div>`;
        }
      }
    ...
    try {
      ...
      // sections is an array of { timestamp_ms, section_type, label }
      if (sections && Array.isArray(sections) && sections.length > 0) {
        const list = sections
          .map(
            (s) =>
              `${formatTimeMs(s.timestamp_ms)} - ${s.label || s.section_type}`,
          )
          .join("\n");
